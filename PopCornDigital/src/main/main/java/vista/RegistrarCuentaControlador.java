...
public class RegistrarCuentaControlador implements Initializable {
    ...
    private void registrarCuenta(ActionEvent event) {
        ...
        if (daoValidacion.existeCorreo(correo) || daoAdminValidacion.existeCorreo(correo)) {
            mostrarAlertaPersonalizada(Alert.AlertType.ERROR, "Error de Registro",
                    "El correo introducido ya está registrado en nuestra base de datos.");
            return;
        }
    ...
    private void mostrarAlertaPersonalizada(Alert.AlertType tipo, String titulo, String contenido) {
        ...
        try {
            ...
            // Si falla la carga del CSS, la alerta se muestra sin estilo pero funcional
        }
        alert.showAndWait();
    }
